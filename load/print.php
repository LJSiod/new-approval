...
// Populate template
$template = new TemplateProcessor('../Details_Template.docx');
...
$template->setImageValue('frontledger', array('path' => $row['front'] ?? '../assets/image/13.png', 'width' => 650, 'height' => 360, 'ratio' => false));
$template->setImageValue('backledger', array('path' => $row['back'] ?? '../assets/image/13.png', 'width' => 650, 'height' => 360, 'ratio' => false));

// Save template
$template->saveAs('../details/' . $row['BranchName'] . $row['ApprovalID'] . '.docx');
$docxFile = realpath('../details/' . $row['BranchName'] . $row['ApprovalID'] . '.docx');
$pdfFile = realpath('../details/') . '/' . $row['BranchName'] . $row['ApprovalID'] . '.pdf';
